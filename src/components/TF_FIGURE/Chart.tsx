...
const GetBarChart: React.FC<Props> = (props) => {
  ...
  const refetch = useCallback(() => {
    return queryClient.fetchQuery(["indicatorDataBarChart", registerShortName])
  }, [queryClient, registerShortName])
  ...
  const data = barChartData
    .filter((data: StatisticData) => !(data.ind_id !== props.description.id ||
      (data.dg ?? 1) < 0.6 ||
    ...
    .map((data: StatisticData) => {
      ...
      if (data.unit_name === "Nasjonalt") {
        ...
        if (props.selectedTreatmentUnits.length > 1) {
          style.opacity = unitNames.includes(data.unit_name)
            ? 1
            : 0.5;
        }
      }
...
const GetLineChart: React.FC<Props> = (props) => {
  ...
  const refetch = useCallback(() => {
    return queryClient.fetchQuery(["indicatorDatalineChart", registerShortName])
  }, [queryClient, registerShortName])

  useEffect(() => {
    refetch()
  }, [unitNameString, refetch])

  const data: DataPoint[] = (lineChartQuery.data ?? [])
    .filter((data: StatisticData) => {
      if (data.unit_level !== "nation") {
        return !(data.ind_id !== props.description.id ||
          (data.dg ?? 1) < 0.6 ||
          data.denominator < (description.min_denominator ?? 5))
      } else {
        return !(data.ind_id !== props.description.id ||
