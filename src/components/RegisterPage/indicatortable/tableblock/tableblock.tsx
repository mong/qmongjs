...
import { filterOrderIndID, validateTreatmentUnits } from '../../../../helpers/functions'
...
export const TableBlock: React.FC<TableBlockProps> = (props) => {
  ...
  const isFetching = queryClient.getQueryState(["indicatorData", registerName.rname])?.isFetching
  ...
  const uniqueOrderedInd: string[] = useMemo(() => filterOrderIndID(
    isFetching ?? false,
    unitNames,
  ...
  if ((descriptionQuery.data.length === 0 || indicatorDataQuery.data.length === 0)) {
    return null
  }
  ...
  const indicatorRows = uniqueOrderedInd.map((indicator) => {
    const singleIndicatorData = indicatorDataQuery.data
      .filter((data: StatisticData) => data.ind_id === indicator)
    const singleIndicatorDescription = descriptionQuery.data
      .filter((data: Description) => data.id === indicator)
    return (
      <IndicatorRow
