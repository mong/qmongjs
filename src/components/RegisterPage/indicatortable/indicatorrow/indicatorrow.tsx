  ...
  if (!indicator[0]) {
    ...
    return (
      <td
        ...
        className={`${indicator[0].unit_level !== "nation" ? style.selected_unit : style.nationally}`}
      >
    ...
    const level_class =
      indicator[0].level !== showLevelFilter &&
        !!showLevelFilter
        ? "filtered_level"
        : "";
    return (
...
export const IndicatorRow: React.FC<IndicatorRowProps> = (props) => {
  ...
  const indPerUnit = unitNames.length === 0
    ? null
    : unitNames.map(name => {
      const filteredIndicator = indicatorData.filter(ind => ind.unit_name === name)
      return formatIndicatorValues(description, filteredIndicator, showLevelFilter, name)
    })

  const indNation = indicatorData.filter(d => d.unit_name === "Nasjonalt")[0]
  const level_class =
    indNation.level !== showLevelFilter &&
      !!showLevelFilter
      ? "filtered_level"
      : "";
