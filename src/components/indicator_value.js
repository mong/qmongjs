...
function INDICATOR_VALUE(props) {
  ...
  const icon_class = data[data_config.column.achieved_level] === "H" 
    ? "fa fa-fas fa-circle"
  ...
  if (ind_type === data_config.indicator_type.andel.db ){
    ...
    const numerator = Math.round(data[data_config.column.variable] * denominator)
    const indicator_value_share = data[data_config.column.variable] < 0.1 
    ...
    return(
      <td className = {td_class}>
        <div className="level">
          <h4 >{`${indicator_value_share} `}<i className={icon_class}></i></h4>
        </div>
